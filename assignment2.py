...
def main():
    ...
    try:
        ...
        # print('Task 1')
        """users = program.runQuery('SELECT COUNT(id) FROM User;', read=True)
        activities = program.runQuery('SELECT COUNT(id) FROM Activity;', read=True)
        trackpoints = program.runQuery('SELECT COUNT(id) FROM TrackPoint;', read=True)

        print(tabulate(users, headers=['Users'], tablefmt='psql'))
        print(tabulate(activities, headers=['Activities'], tablefmt='psql'))
        print(tabulate(trackpoints, headers=['Trackpoints'], tablefmt='psql'))"""
        ...
        print(tabulate(result, headers=['Average', 'Minimum', 'Maximum'], tablefmt='psql'))"""
        ...
        print(tabulate(result, headers=['Users who have taken a bus'], tablefmt='psql'))"""
        ...
        print(tabulate(result, headers=['User', 'Transportation modes'], tablefmt='psql'))"""
        ...
        print(tabulate(result, headers=['Activity'], tablefmt='psql'))"""
        ...
        print(tabulate(result, headers=['No. of users'], tablefmt='psql'))"""
        ...
        # LIMIT 15
        # ''', read=True)
        # print(tabulate(result, headers=['User ID', 'Altitude gained'], tablefmt='psql'))
        ...
        """print('Task 11')
        print(tabulate(program.runQuery('''
        SELECT
            subquery.user_id, COUNT(DISTINCT subquery.activity_id) AS unique_activity_count
        FROM
            (
                SELECT
                a.user_id,
                a.id AS activity_id,
                TIMESTAMPDIFF(MINUTE, tp1.date_time, tp2.date_time)
            FROM
                Activity AS a
            JOIN
                TrackPoint AS tp1 ON a.id = tp1.activity_id AND a.user_id = tp1.user_id
            JOIN
                TrackPoint AS tp2 ON tp1.id = tp2.id - 1 AND tp2.activity_id = a.id
            WHERE
